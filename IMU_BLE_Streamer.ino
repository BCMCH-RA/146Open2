...
void loop() {
    ...
    // Update labels ~5 Hz; avoids constant label invalidations at loop rate
    if (now - lastDispUpdate >= 200) {
        lastDispUpdate = now;
        Lvgl_Update_Display(dispIST, dispDate, dispBLE, dispRec, totalSamples, batVolts, dispSev, dispSevColor, tremorRms);
    }
